# Petrov Lab project. I recommend opening this file with RStudio for optimal reading convenience.
# This is the final subfunction for plotting jitter plot. This subfunction will be called by other programs for data analysis.
...
JitterTumor <- function(Data, FileName="Test.pdf", CellCutoff = 200, color="Default", scale = 3, Inert = c("Neo1","Neo2","Neo3","NT1")){
  ...
  sgIDAll <- sort(unique(Data$target))
  ...
  for(i in 1:length(sgIDAll)){
    Data1 <- Data$CellNum[Data$target == sgIDAll[i]]
...
Data <- read.csv("S23_BarcodeCleanCellNum.txt", header = T, sep="\t", stringsAsFactors = F)
JitterTumor(Data, FileName = "Point size S23 subfunction.pdf", CellCutoff = 500)
...
# # Now plot using S25_BarcodeCleanCellNum.txt file.
Data <- read.csv("KTCtest.txt", header = T, sep="\t", stringsAsFactors = F)
JitterTumor(Data, FileName = "KTC size test.pdf", CellCutoff = 200)
