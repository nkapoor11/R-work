...
JitterTumor <- function(Data, FileName="Test.pdf", CellCutoff = 200, color="Default", scale = 2, Inert = c("Neo1","Neo2","Neo3","NT1")){
...
range <- c(5)
chuanCode <- c()
sampleData <- read.csv("sampleDataxls copy 2.txt", header = T, sep = "\t", stringsAsFactors = F)
Inert = c("Neo1", "Neo2", "Neo3", "NT1")
for (i in range) {
  iString <- toString(i)
  for (j in 9:9) {
    if (i == 4 && j >= 20) {
    ...
    # get pdf name as string
    d <- sampleData[sampleData$Chuan.Code == chuanCode, ]
    exp <- (d$Experiment)
    tr <- (d$Treatment)

    pdfName <- paste(c(exp, " - ", tr, " - ", chuanCode, ".pdf"), collapse = '')
    print(fileName)
    # create plots
    Data <- read.csv(fileName, header = T, sep = "\t", stringsAsFactors = F)
    JitterTumor(Data, FileName = pdfName, CellCutoff = 500, Inert=Inert)
  }
}
